object Form15: TForm15
  Left = 279
  Top = 217
  Width = 603
  Height = 378
  Caption = #1056#1045#1044#1040#1050#1058#1048#1056#1054#1042#1040#1053#1048#1045' '#1058#1040#1041#1051#1048#1062#1067' "'#1050#1054#1053#1058#1056#1054#1051#1068'"'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 587
    Height = 340
    Align = alClient
    Color = 13814453
    TabOrder = 0
    object Label1: TLabel
      Left = 192
      Top = 48
      Width = 78
      Height = 16
      Caption = #8470' '#1055#1056#1054#1045#1050#1058#1040
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label2: TLabel
      Left = 216
      Top = 88
      Width = 54
      Height = 16
      Caption = #1054#1041#1066#1045#1050#1058
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 160
      Top = 128
      Width = 110
      Height = 16
      Caption = #1040#1044#1056#1045#1057' '#1054#1041#1066#1045#1050#1058#1040
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label4: TLabel
      Left = 168
      Top = 168
      Width = 103
      Height = 16
      Caption = #1042#1048#1044' '#1050#1054#1053#1058#1056#1054#1051#1071
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label5: TLabel
      Left = 24
      Top = 208
      Width = 240
      Height = 16
      Caption = #1057#1058#1040#1044#1048#1071' '#1043#1054#1058#1054#1042#1053#1054#1057#1058#1048' '#1047#1040#1050#1051#1070#1063#1045#1053#1048#1071
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DBEdit1: TDBEdit
      Left = 296
      Top = 40
      Width = 265
      Height = 24
      DataField = #8470'_'#1087#1088#1086#1077#1082#1090#1072
      DataSource = DataModule2.DataSource3
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object DBEdit2: TDBEdit
      Left = 296
      Top = 80
      Width = 265
      Height = 24
      DataField = #1054#1073#1098#1077#1082#1090
      DataSource = DataModule2.DataSource3
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
    object DBEdit3: TDBEdit
      Left = 296
      Top = 120
      Width = 265
      Height = 24
      DataField = #1040#1076#1088#1077#1089'_'#1086#1073#1098#1077#1082#1090#1072
      DataSource = DataModule2.DataSource3
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
    end
    object BitBtn1: TBitBtn
      Left = 176
      Top = 288
      Width = 113
      Height = 25
      Caption = #1057#1054#1061#1056#1040#1053#1048#1058#1068
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        8738288738247D3367A567B7CEB7FFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object BitBtn2: TBitBtn
      Left = 320
      Top = 288
      Width = 113
      Height = 25
      Caption = #1054#1058#1052#1045#1053#1040
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFE4E3FE9E9BF96566F66D6CF7}
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 296
      Top = 160
      Width = 265
      Height = 24
      DataField = 'Vidcontrolya'
      DataSource = DataModule2.DataSource3
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
    end
    object DBLookupComboBox2: TDBLookupComboBox
      Left = 296
      Top = 200
      Width = 265
      Height = 24
      DataField = 'Stadiya'
      DataSource = DataModule2.DataSource3
      Font.Charset = RUSSIAN_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Sitka Text'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
    end
  end
end
